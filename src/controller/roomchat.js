...
module.exports = {
    ...
    try {
      ...
      if (result.length > 0) {
        ...
        return helper.response(
          response,
          404,
          `Message By Id : ${user_id} Not Found`
        );
      }
    } catch (error) {
    ...
    try {
      ...
      if (result.length > 0) {
        return helper.response(
          response,
          200,
          "Succes get Roomchat By User Id",
          result
        );
      } else {
        return helper.response(
    ...
    try {
      ...
      if (result.length > 0) {
        return helper.response(
          response,
          200,
          "Succes get Message By Roomchat Id",
          result
        );
      } else {
        return helper.response(
          response,
          404,
          `Message By Id : ${roomchat_id} Not Found`
        );
      }
    } catch (error) {
      return helper.response(response, 400, "Bad Request", error);
  ...
  postRoomChat: async (request, response) => {
    try {
      const { user_id, msg } = request.body;
      if (
